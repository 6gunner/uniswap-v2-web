...
const Option = styled(FancyButton)<{ active: boolean }>`
  ...
  background-color: ${({ active, theme }) => active && theme.primaryText1};
  color: ${({ active, theme }) => (active ? theme.white : theme.text6)};
`;
...
const OptionCustom = styled(FancyButton)<{
  ...
  border: ${({ theme, active, warning }) =>
    active && `1px solid ${warning ? theme.red1 : theme.primary3}`};
  :hover {
    border: ${({ theme, active, warning }) =>
      active && `1px solid ${warning ? darken(0.1, theme.red1) : darken(0.1, theme.primary3)}`};
  }
    ...
    <AutoColumn gap="md">
      <AutoColumn gap="sm">
        <RowFixed>
          ...
          <QuestionHelper
            isWhite={false}
            text="Your transaction won’t go through if the price moves past your slippage buffer."
          />
        </RowFixed>
        <RowBetween>
          <Option
