...
import { getShareValueOverTime } from "./swapInfo";
...
interface ReturnMetrics {
  ...
  sugarFinanceReturn: number; // netReturn - hodlReturn
  impLoss: number;
  fees: number;
}
...
interface SugarFinance {
  return: number;
}

// 用户position和token pair的快照数据合并后的数据
export interface Position {
  fees: Fees;
  liquidityTokenBalance: string;
  net: Net;
  pair: Pair;
  principal: Principal; // 显示您最初投入了多少ETH和USDC;
  sugarFinance: SugarFinance;
}
...
// 修改一些token的价格
function formatPricesForEarlyTimestamps(position: any): LiquidityPositionSnapshot {
...
export function getMetricsForPositionWindow(
  positionT0: LiquidityPositionSnapshot,
  positionT1: LiquidityPositionSnapshot
): ReturnMetrics {
  ...
  const sugarFinance_return = difference_fees_usd + imp_loss_usd;
  ...
  return {
    ...
    sugarFinanceReturn: sugarFinance_return,
...
export async function getHistoricalPairReturns(
  ...
  pairSnapshots: LiquidityPositionSnapshot[],
    ...
    if (positionT1) {
      ...
      const currentLiquidityValue =
        (parseFloat(positionT1.liquidityTokenBalance) /
          parseFloat(positionT1.liquidityTokenTotalSupply)) *
...
export async function getLPReturnsOnPair(
  ...
  const currentPosition: LiquidityPositionSnapshot = {
    pair,
  ...
  for (const index in snapshots) {
    ...
    sugarFinanceReturn = sugarFinanceReturn + results.sugarFinanceReturn;
