...
import { Trade, TokenAmount, CurrencyAmount, ETHER } from "@repo/sugar-finance-sdk";
    ...
    return tokenContract
      ...
      .then((response: TransactionResponse) => {
        addTransaction(response, {
          ...
          approval: { tokenAddress: token.address, spender: spender },
          title: ["Approved", "Approved Failed"],
          currency0Symbol: amountToApprove.currency.symbol,
        });
        setIsLoading(false);
      })
      .catch(() => setIsLoading(false));
  }, [approvalState, token, tokenContract, amountToApprove, spender, addTransaction, isLoading]);
